...
@section('content')
    ...
    <div class="text-center">
        ...
        <p class="text-muted mt-2">{{ __('Before proceeding, please check your email for a verification link. If you did not receive the email') }}</p>

        <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
            @csrf
            <div class="d-grid">
                <button type="submit" class="btn btn-primary waves-effect waves-light mt-3">{{ __('click here to request another') }}</button>
            </div>
        </form>
    </div>
    ...
    <footer class="footer footer-alt">
        <p class="text-muted">
            Don't have an account? <a href="{{ route('register') }}" class="text-primary fw-medium ms-1">{{ __('Register') }}</a>
        </p>
    </footer>
@endsection
